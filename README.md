...
팝업 UI는 "디지털 다이어리/집" 감성(픽셀 폰트 제목, 점선 눈금자, 흐릿한 대형 번호가 깔린 섹션 헤더, 각진 카드와 hover-invert 버튼)을 모티브로 삼았습니다. 색상은 두 가지 그린 팔레트를 라이트/다크 테마에 각각 매핑했습니다: 선명한 그린~블랙 팔레트는 포인트 컬러와 다크 모드 배경에, 차분한 세이지~틸 그러데이션은 라이트 모드 배경/보더에 사용했습니다.
