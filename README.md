...
- 라이트/다크 모드 토글 (기본은 OS 설정을 따르고, 헤더의 버튼으로 직접 전환 가능)
- 설정은 `chrome.storage.sync`에 저장되어 로그인된 크롬 계정 간 동기화됩니다

## 디자인

팝업 UI는 개인 홈페이지 [fcfcfcfc.dothome.co.kr](https://fcfcfcfc.dothome.co.kr/)의 "디지털 다이어리/집" 감성(픽셀 폰트 제목, 점선 눈금자, 흐릿한 대형 번호가 깔린 섹션 헤더, 각진 카드와 hover-invert 버튼)을 모티브로 삼았습니다. 색상은 두 가지 그린 팔레트를 라이트/다크 테마에 각각 매핑했습니다: 선명한 그린~블랙 팔레트는 포인트 컬러와 다크 모드 배경에, 차분한 세이지~틸 그러데이션은 라이트 모드 배경/보더에 사용했습니다.
